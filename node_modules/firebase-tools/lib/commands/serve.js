...
var filterOnlyEmulators = function (only) {
    if (!only) {
        return [];
    }
    return _.intersection(VALID_EMULATORS, only.split(",").map(function (opt) {
        return opt.split(":")[0];
    }));
};
module.exports = new Command("serve")
    ...
    return requireConfig(options)
        .then(function () { return requirePermissions(options); })
        .then(function () { return checkDupHostingKeys(options); })
        .then(function () { return getProjectNumber(options); });
})
