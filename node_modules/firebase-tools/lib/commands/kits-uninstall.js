...
function _getFunctions(dict, kitName) {
    return _.reduce(dict[kitName], function (funcs, func) {
    ...
    return prompt({}, [
        {
            type: "checkbox",
            name: "kitNames",
            message: "Which kits would you like to delete? " +
...
function _deleteKitFunctions(projectId, functions) {
    return Promise.all(_.map(functions, function (funcName) {
...
module.exports = new Command("kits:uninstall [kitName]")
    ...
    return _listKits(projectId)
        ...
        utils.logBullet(clc.cyan.bold("kits: ") + "Checking to make sure kits have been deleted safely...");
        var printSuccess = function (kits) {
            return utils.logSuccess(clc.green.bold("kits: ") +
                "Successfully deleted the following kit(s): " +
                clc.bold(_.uniq(kits)));
        };
        var printFail = function (reason) {
            return utils.logWarning(clc.yellow.bold("kits: ") + "Failed to delete the following kit: " + reason);
        };
        return pollKits(operations, printSuccess, printFail);
    });
});
//# sourceMappingURL=kits-uninstall.js.map