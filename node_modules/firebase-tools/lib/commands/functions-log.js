...
module.exports = new Command("functions:log")
    ...
    if (options.open) {
        var url = "https://console.developers.google.com/logs/viewer?advancedFilter=" +
            consoleFilter +
            "&project=" +
            projectId;
        open(url);
    ...
    return gcp.cloudlogging
        ...
        for (var i = _.size(entries); i-- > 0;) {
            var entry = entries[i];
            logger.info(entry.timestamp, entry.severity.substring(0, 1), entry.resource.labels.function_name + ":", entry.textPayload);
        }
        ...
        .catch(function (err) {
        return Promise.reject(new FirebaseError("Failed to list log entries " + err.message, {
            exit: 1,
        }));
    });
});
