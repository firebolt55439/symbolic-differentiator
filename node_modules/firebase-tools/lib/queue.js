...
var Queue = (function () {
    ...
    Queue.prototype.taskName = function (task) {
    ...
    Queue.prototype.process = function () {
        if (this._finishIfIdle() ||
            this.active >= this.concurrency ||
            this.cursor === this.tasks.length) {
            return;
        }
        var task = this.tasks[this.cursor];
        this.cursor++;
        this.active++;
        this.handle(task);
    };
    Queue.prototype.handle = function (task) {
        var t0 = Date.now();
        var self = this;
        this.handler(task)
            .then(function () {
            ...
            if (self.retryCounts[task] > 0) {
                logger.debug("[" + self.name + "] Retries exhausted for task", self.taskName(task), ":", err);
            }
            else {
    ...
    Queue.prototype._finishIfIdle = function () {
    ...
    Queue.prototype._finish = function (err) {
        var self = this;
